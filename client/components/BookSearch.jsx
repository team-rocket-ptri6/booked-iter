...
function BookSearch({ idList, setUpdate, updateList }) {
  ...
  const fetchBooks = (query) => {
    if (query === '') return;
    query = query.replace(' ', '+');
    return fetch(`https://www.googleapis.com/books/v1/volumes?q=intitle:${query}&maxResults=10&sorting=relevance&projection=lite&fields=items(id,volumeInfo(title, authors,imageLinks(thumbnail)))`)
